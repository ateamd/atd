<div class="menugs">
    <div class="center">
<ul class="nav">
        <li id="hm">
            <a href="http://atd.adwmicro.pl" ><img src="img/section-menu-img/logoatd.png" itemprop="image" class="logo"></a>
        </li>
        <li id="st">
            <a href="javascript:jumpScroll1()"><?php echo $lang['home']; ?></a>
        </li>
        <li id="au">
            <a href="javascript:jumpScroll2()"><?php echo $lang['about']; ?></a>
        </li>
        <li id="pf">
            <a href="javascript:jumpScroll3()"><?php echo $lang['portfolio']; ?></a>
        </li>
        <li id="cs">
            <a href="javascript:jumpScroll4()"><?php echo $lang['clients']; ?></a>
        </li>
        <li id="ct">
            <a href="javascript:jumpScroll5()"><?php echo $lang['contact']; ?></a>
        </li>
       


        <li>
            <a href="#"><?php echo $lang['language'] ; ?> ▼&nbsp;</a>
            <ul>
                <li><a href="?lang=pl" >PL</a></li>
                <li><a href="?lang=ang">ENG</a></li>
            </ul>
        </li>
    </ul>
</div>
</div>
      
    <script type="text/javascript">
     function jumpScroll1() {
    window.scroll(0,0); 
    }
    </script>
    <script type="text/javascript">
    function jumpScroll2() {
    window.scroll(0,700); 
    }
    </script>
    <script type="text/javascript">
    function jumpScroll3() {
    window.scroll(0,1600); 
    }
    </script>
    <script type="text/javascript">
    function jumpScroll4() {
    window.scroll(0,2500); 
    }
    </script>
    <script type="text/javascript">
    function jumpScroll5() {
    window.scroll(0,150000); 
    }
    </script>

